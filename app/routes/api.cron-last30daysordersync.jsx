...
async function getCronAdminContext(shopDoc) {
  ...
  if (shopDoc.accessToken) {
    console.warn(
      `[cron-last30] using stored MongoDB access token for ${shopDoc.shopId} because no Prisma offline session was found`,
    );
...
async function handleCronRequest(request) {
  ...
  console.log(`[cron-last30] sync started for ${shops.length} shop(s)`);
  ...
  for (const shopDoc of shops) {
    ...
    try {
      ...
      console.error(`[cron-last30] failed for ${shop}:`, error);
      results.push({
        shop,
        status: "error",
        error: error.message,
  ...
  console.log(`[cron-last30] sync completed: ${processed} succeeded, ${failed} failed`);
